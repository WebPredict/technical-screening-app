  ...
  def home
    if logged_in?
      ...
      if current_user.trial_expired?
        log_out
        flash[:info] = "Your 2 week trial has expired. Please sign up for either the Bronze, Gold, or Platinum subscription to continue using WebPredict."
        ...
        if @num_tests == 0 && @num_candidates == 0 && !current_user.companies.any?
          flash.now[:info] = "Welcome to WebPredict! You can view all of your screening results and candidates on this dashboard."
          @is_new_user = true
        end
      end      
    else
