...
question19 = Question.create!(content: "What is meant by Markup?", answer: "<p>Markup is the double curly brace notation <strong>{{}}</strong> to bind expressions to elements.  For example:</p><pre><code>  &lt;button ng-click=&quot;changeFoo()&quot;&gt;{{buttonText}}&lt;/button&gt;</code></pre><h4 id="-what-are-singletons-"><strong><%=@ctr+=1 %>. What are singletons?</strong></h4><p>In object-oriented programming, specifically, singletons are classes that are instantiated once.  They have only one instance or object created from the class.  An example of a singleton would be an &quot;Application&quot; class, as only one object can be created of an Application class.</p>", user_id: 1, difficulty_id: 2, category_id: 2)
question20 = Question.create!(content: "What are the issues that may be present when using singletons?", answer: "<p>Singletons introduce global state into a program, allowing them to be accessed by anyone at anytime.  They are often used, unnecessarily, by unskilled or thoughtless programers.  This can create programs that are hard to test and that hide their dependencies.</p>", user_id: 1, difficulty_id: 2, category_id: 2)
question21 = Question.create!(content: "What are services in AngularJS?", answer: "<p>Services in AngularJS are essentially singletons, passed around, to ensure that the programmer is dealing with the same object each time.  It allows various controllers or directives to affect its values, and it is how the programmer shares date between different chunks of code in the application. The following is an example of a service:</p><pre><code>module.service( &#39;Car&#39;, [ &#39;$rootScope&#39;, function( $rootScope ) {   var service = {     cars: [       { model: &quot;Civic&quot;, manufacturer: &quot;Honda&quot; },       { model: &quot;Focus&quot;, manufacturer: &quot;Ford&quot; }     ],     addCar: function ( car ) {       service.cars.push( car );       $rootScope.$broadcast( &#39;cars.update&#39; );     }   }   return service; }]);</code></pre><h4 id="-what-is-a-controller-provide-an-example-"><strong><%=@ctr+=1 %>. What is a controller? Provide an example.</strong></h4><p>Controllers in AngularJS are &quot;classes&quot; or &quot;constructor functions&quot; that control the data of an AngularJS application. They are regular JavaScript objects that provide the application behavior that supports the declarative markup in the template.  For example:</p><pre><code>someModule.controller(&#39;MyController&#39;, [&#39;$scope&#39;, &#39;dep1&#39;, &#39;dep2&#39;, function($scope, dep1, dep2) {...$scope.aMethod = function() {  ...}...}]);</code></pre><h4 id="-how-should-controllers-be-used-"><strong><%=@ctr+=1 %>. How should controllers be used?</strong></h4><p>Controllers should be used to:</p><ul><li>Set up the initial state of the $scope object.</li><li>Add behavior to the $scope object.</li></ul><p>Controllers should not be used to:</p><ul><li>Manipulate the DOM.  Controllers should contain only business logic not presentation logic.</li><li>Format input.</li><li>Filter output.</li><li>Share code or state across controllers.</li><li>Manage the life-cycle of other components.</li></ul>", user_id: 1, difficulty_id: 2, category_id: 2)
question22 = Question.create!(content: "Explain what business logic is and where it goes in Angular.", answer: "<p>In software engineering, business logic or domain logic does the actual work of the program.  It determines how data is created, displayed, stored, and modified. The controller and services would contain the business logic, typically. </p>", user_id: 1, difficulty_id: 2, category_id: 2)
question23 = Question.create!(content: "What is the $inject property?  Explain its function.", answer: "<p>The <strong>$inject</strong> property is an array of service names to inject. It annotates a function to allow minifiers to rename the function parameters and still inject the proper services.    For example:</p><pre><code>var MyController = function($scope, greeter) {// ...}MyController.$inject = [&#39;$scope&#39;, &#39;greeter&#39;];someModule.controller(&#39;MyController&#39;, MyController);</code></pre><h4 id="-what-is-a-factory-method-how-are-they-declared-"><strong><%=@ctr+=1 %>. What is a factory method? How are they declared?</strong></h4><p>Factory methods define directives, services, and filters and are registered with modules.  It is recommended that they be declared as follows:</p><pre><code>angular.module(&#39;myModule&#39;, []).factory(&#39;serviceId&#39;, [&#39;depService&#39;, function(depService) {  // ...}]).directive(&#39;directiveName&#39;, [&#39;depService&#39;, function(depService) {  // ...}]).filter(&#39;filterName&#39;, [&#39;depService&#39;, function(depService) {  // ...}]);</code></pre>", user_id: 1, difficulty_id: 2, category_id: 2)
question24 = Question.create!(content: "What is Dependency Injection?", answer: "<p>Dependency Injection is a software design pattern where instead of having  objects create a dependency or asking a factory object to make one for them, the needed dependencies are given to components through their constructors, methods, or directly into fields.</p>", user_id: 1, difficulty_id: 2, category_id: 2)
question25 = Question.create!(content: "How is dependency injection used in AngularJS?", answer: "<p>It can be used when defining components or when providing <strong>run</strong> and <strong>config</strong> blocks for a module.  For example:</p><ul><li>Components can be defined by an injectable factory method or constructor function. These components can be injected with &quot;service&quot; and &quot;value&quot; components as dependencies.</li><li>Controllers are defined by a constructor function, which can be injected with any of the &quot;service&quot; and &quot;value&quot; components as dependencies, but they can also be provided with special dependencies.</li><li>The <strong>run</strong> method accepts a function, which can be injected with &quot;service&quot;, &quot;value&quot; and &quot;constant&quot; components as dependencies.</li><li>The <strong>config</strong> method accepts a function, which can be injected with &quot;provider&quot; and &quot;constant&quot; components as dependencies. </li></ul>", user_id: 1, difficulty_id: 2, category_id: 2)
question26 = Question.create!(content: "What are module methods in AngularJS?  Provide an example.", answer: "<p>These are functions that can be specified to run at configuration and runtime for a module by calling the <strong>config</strong> and <strong>run</strong> methods.  They are also injectable with dependencies.  For example:</p><pre><code>angular.module(&#39;myModule&#39;, []).config([&#39;depProvider&#39;, function(depProvider) {  // ...}]).run([&#39;depService&#39;, function(depService) {  // ...}]);</code></pre>", user_id: 1, difficulty_id: 2, category_id: 2)
...
test2 = Test.create!(name: "Basic Angular Test", description: "This is the basic Angular test.", user_id: 1)
test2.questions << question1
test2.questions << question2
test2.questions << question3
test2.questions << question4
test2.questions << question5
test2.save

question1 = Question.create!(content: "What types of inheritance does Java support?", answer: "<p>Java supports multiple interface inheritance, but not multiple implementation (class) inheritance. This means a class in Java can have only one base class, but has no limit on implemented interfaces (i.e. promises to provide certain functionality).</p>", user_id: 1, difficulty_id: 2, category_id: 21)
question2 = Question.create!(content: "Explain equals and hashcode method usage, and how they relate to each other.", answer: "<p>They must be logically in sync to ensure consistent, expected behavior when objects are used in collections and comparisons. More specifically, when two objects are considered equal, their hash codes should be the same. When they differ, their hashcodes should be different, in a consistent way.</p>", user_id: 1, difficulty_id: 2, category_id: 21)
question3 = Question.create!(content: "How would you increase the heap size on startup?", answer: "<p>Pass the desired maximum heap size to the Java VM property like so:</p><pre>java -Xmx1024m ...</pre><p>The -Xmx parameter here specifies max heap size (in this example in megabytes).</p>", user_id: 1, difficulty_id: 2, category_id: 21)
question4 = Question.create!(content: "What are generics?", answer: "<p>Generics are classes that are parameterized by a specific type. They enable code reuse and strong typing (type safety at compile time), and are often seen when using collection data structures in Java. For example:</p><pre>HashMap&lt;String, Integer&gt; nameToAgeMap = new HashMap&lt;String, Integer&gt;();nameToAgeMap.put("John Smith", 55); // OK, checked at compile timenameToAgeMap.put("John Smith", "55");  // Will not compile as the types do not match, since "55" is a String, not an Integer</pre>", user_id: 1, difficulty_id: 2, category_id: 21)
question5 = Question.create!(content: "What's a TreeMap used for? When would you use it instead of a HashMap?", answer: "<p>A map with a deterministic sorted set of keys. For example:</p><pre>	TreeMap&lt;String,String&gt; sortedMap = new TreeMap&lt;String,String&gt;();	sortedMap.put("FirstKey", "value1");	sortedMap.put("AnotherKey", "value2");	sortedMap.put("Third", "value3");	for (String key : sortedMap.keySet()) {		System.out.println("key is: " + key);	}	// Will print:	// AnotherKey	// FirstKey	// Third	// every time.</pre>", user_id: 1, difficulty_id: 2, category_id: 21)
question6 = Question.create!(content: "What's the difference between unchecked and checked exceptions?", answer: "<p>Checked exceptions must be explicitly handled if they are declared to be thrown from a particular method. Unchecked do not.</p>", user_id: 1, difficulty_id: 2, category_id: 21)
question7 = Question.create!(content: "What does autoboxing/unboxing refer to?", answer: "<p>It refers to the process of automatically converting primitive types to/from their object equivalents. For example:</p><pre>int primitive = 5;Integer objectInt = primitive; // autoboxing happened</pre>", user_id: 1, difficulty_id: 2, category_id: 21)
